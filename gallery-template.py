...
def getclose():
  pathToFile='closebtn.png'
  return static_file(pathToFile, root='img')

@route('/btn/prev') # url картинки "предыдущая"
def getprev():
  pathToFile='arrow_LeftLight.png'
  return static_file(pathToFile, root='img')

@route('/btn/next') # url картинки "следующая"
def getnext():
  pathToFile='arrow_RightLight.png'
  return static_file(pathToFile, root='img')
...
def big_html(id):
  ...
  html=html+ '<div id="bigpage"><div class="container">' + prevbtn +  '<div class="big-picture"> <img src="/small/' +str(id) +  '.jpg">' +'<p>%s</p>' + '</div></div>' + nextbtn +'</body></html>'
